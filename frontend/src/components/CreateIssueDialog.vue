...
const props = defineProps<{ open: boolean; projectKey: string }>();
const emit = defineEmits<{ (e: "close"): void; (e: "created", message: string): void }>();
...
async function submit() {
  ...
  try {
    ...
    const { data: response, status } = await IssueService.createIssue(
      props.projectKey,
      payload
    );
    if (status === 201) {
      if (createOther.value) {
        ...
        form.value.originalEstimate = null;
        form.value.remainingEstimate = null;
        form.value.timeSpent = null;
        return;
      }
      ...
      <div v-else class="space-y-3">
        ...
        <div class="flex justify-between">
          <div class="flex items-center">
            <input type="checkbox" id="createOther" v-model="createOther" />
            <label class="text-sm ml-2" for="createOther"> Create another</label>
          </div>
